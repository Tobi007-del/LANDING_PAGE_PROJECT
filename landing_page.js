...
let observer = new IntersectionObserver((entries, observer) => {
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      if (
        entry.target.dataset.identity === "accreditation" &&
      ...
      if (
        entry.target.dataset.identity === "population-counter" &&
        entry.intersectionRatio > 0.2
      ) {
        fCount++;
        if (fCount === 1) {
...
let counterElements = document.querySelectorAll(
  "#accreditation-section-container,.population-content-wrapper",
);
